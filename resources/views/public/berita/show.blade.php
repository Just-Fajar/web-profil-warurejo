...
<section class="py-6 sm:py-8 md:py-12 bg-gray-50">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto">
            ...
            {{-- Article Header --}}
            <article class="bg-white rounded-lg shadow-lg overflow-hidden mb-6 sm:mb-8 scroll-reveal">
                {{-- Featured Image --}}
                ...
                <div class="p-4 sm:p-6 md:p-8">
                    {{-- Category & Date --}}
                    <div class="flex flex-wrap items-center gap-2 sm:gap-3 md:gap-4 mb-4 sm:mb-6">
                        <span class="px-3 sm:px-4 py-1.5 sm:py-2 bg-primary-100 text-primary-700 text-xs sm:text-sm font-semibold rounded-full">
                            Berita Desa
                        </span>
                        <div class="flex items-center text-gray-600 text-xs sm:text-sm shrink-0">
                    ...
                    @if(isset($berita->tags) && $berita->tags)
                        <div class="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-gray-200">
                            <div class="flex items-start">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 mr-2 sm:mr-3 mt-1 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M17.707 9.293a1 1 0 010 1.414l-7 7a1 1 0 01-1.414 0l-7-7A.997.997 0 012 10V5a3 3 0 013-3h5c.256 0 .512.098.707.293l7 7zM5 6a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"/>
                                </svg>
                                <div class="flex flex-wrap gap-2">
                                    @foreach(explode(',', $berita->tags) as $tag)
                                        <span class="px-2.5 sm:px-3 py-1 bg-gray-100 text-gray-700 text-xs sm:text-sm rounded-full">
                                            {{ trim($tag) }}
                                        </span>
                                    @endforeach
                                </div>
                    ...
                    {{-- Share Buttons --}}
                    <div class="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-gray-200">
                        <p class="text-gray-700 font-semibold mb-3 sm:mb-4 text-sm sm:text-base">Bagikan:</p>
                        <div class="flex flex-wrap gap-2 sm:gap-3">
                            <a 
                                href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('berita.show', $berita->slug)) }}" 
                                target="_blank"
                                class="flex items-center px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-xs sm:text-sm font-medium"
                            >
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-1.5 sm:mr-2 shrink-0" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            <a 
                                ...
                                target="_blank"
                                class="flex items-center px-3 sm:px-4 py-2 bg-sky-500 text-white rounded-lg hover:bg-sky-600 transition text-xs sm:text-sm font-medium"
                            >
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-1.5 sm:mr-2 shrink-0" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            <a 
                                ...
                                target="_blank"
                                class="flex items-center px-3 sm:px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition text-xs sm:text-sm font-medium"
                            >
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-1.5 sm:mr-2 shrink-0" fill="currentColor" viewBox="0 0 24 24">
            ...
            @if(isset($relatedNews) && $relatedNews->count() > 0)
                <section class="mt-8 sm:mt-10 md:mt-12 scroll-reveal">
                    <h2 class="text-xl sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6">Berita Terkait</h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                        @foreach($relatedNews as $related)
                            <a href="{{ route('berita.show', $related->slug) }}" class="group bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition scroll-reveal-stagger">
                                ...
                                <div class="p-3 sm:p-4">
                                    <p class="text-xs text-gray-500 mb-1.5 sm:mb-2">{{ $related->tanggal_publikasi->format('d M Y') }}</p>
                                    <h3 class="font-bold text-sm sm:text-base text-gray-800 group-hover:text-primary-600 transition line-clamp-2">
                                        {{ $related->judul }}
                                    </h3>
                                </div>
                            </a>
            ...
            {{-- Back Button --}}
            <div class="mt-8 sm:mt-10 md:mt-12 text-center scroll-reveal">
                <a 
                    href="{{ route('berita.index') }}" 
                    class="inline-flex items-center px-5 sm:px-6 py-2.5 sm:py-3 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition font-semibold text-sm sm:text-base"
                >
